...
This component is responsible for playing audio requested via the [Event Action](/actions) bus. Internally it will hold duel playlists, and house two players; one for each of the playlists.

The audio-tracks are declaratively defined in HTML to respond to user interactions. Unlike a typical playlist, the audio items are configured with behavior rules that help orchestrate overlapping voice-over and ambient sound.
...
Note: Additionally, it subscribes to the navigation change emitter for route-change notifications - as some audio clips are meant to end when the route changes.

## Audio Event Action Commands
...
### Looping
Only the ambient player will support looping. Default is true. Looping audio loops until it's discard event occurs. 
...
### Discard Strategy
Each audio track-request defines when it should be stopped and removed from the queue. This allows for ambient music to  plays between routes. By default, a route-change will empty the queue of any unplayed audio.
...
* **none**: Play until a new track is played (default for ambient)
