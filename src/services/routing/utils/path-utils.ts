...
export const stripBasename = (path: string, prefix: string) => (hasBasename(path, prefix) ? path.substr(prefix.length) : path);

export const stripTrailingSlash = (path: string) => (path.charAt(path.length - 1) === '/' ? path.slice(0, -1) : path);

export const addLeadingSlash = (path: string) => (path.charAt(0) === '/' ? path : `/${path}`);

export const stripLeadingSlash = (path: string) => (path.charAt(0) === '/' ? path.substr(1) : path);

export const stripPrefix = (path: string, prefix: string) => (path.indexOf(prefix) === 0 ? path.substr(prefix.length) : path);
